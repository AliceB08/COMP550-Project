...
#remove songs that are not in english, lyric punctuation, and section markers (e.g.[verse: 1])
def pre_process(songs):
    indices = list(songs.index)
    for i,song in zip(indices, songs['lyrics']):
        if detect(song) == 'en':
...
#            songs['genre'][i] = genre_to_int(songs['genre'][i])
        #drops songs that are not in english
        else:
            songs.drop(i)
    return 
