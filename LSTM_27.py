...
#load data
training_data = get_data('./train_data.csv', size = 5000)
testing_data = get_data('./test_data.csv', size = 500)
...
#        for i in range(1, 10):
#            pred.append(categoryFromOutput(tag_scores[-i])[0])
#
#        print('predicted label:', pred, 'true label:', targets[0].item())
    #calculate accuracy
    train_acc = 100*train_correct / len(training_data)
    
    print('predicted label:', categoryFromOutput(tag_scores[-5])[0], 'true label:', targets[-5].item())
